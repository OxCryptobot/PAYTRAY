...
| Post-attestation sequence | Read-only ordered release-gate sequencing | Implemented locally | `backend:release:post-attestation:sequence:check` maps target evidence, recovery, verifier freshness, reconciliation, durable workers, human evidence, custody, manifest, and payload stages; the release remains `operator_blocked` until real evidence passes. |
| Attestation bundle | Four-role redacted EIP-191 attestation evidence contract | Implemented locally | `backend:release:reviewer-attestations:bundle:check` requires exactly four verified approved attestations, distinct wallets, shared commit/artifact/fingerprint binding, no raw signatures, and immutable false/read-only safety fields. |
| Durable-worker evidence | Redacted outbox, worker, and idempotency evidence composer | Implemented locally | `backend:release:durable-worker:evidence:check` validates pure JSON reports, SHA-256 source hashes, local-versus-authenticated-target classification, and false/read-only authority fields. |
...
| Post-attestation sequence | Read-only release-gate ordering and CI artifact | Implemented locally | New sequence verifier and release-gates CI artifact preserve ordered blockers and immutable false/read-only safety fields. |
