...
| CC | Centralized release/reconciliation evidence collectors | Validated locally | Shared collectors make the CLI and API use one database-backed evidence contract, with sign-off file parsing, target preflight, verifier, readiness, delivery, and reconciliation assembly. |
| CD | Authenticated operator evidence APIs | Validated locally | `GET /api/v2/ops/release-evidence` and `GET /api/v2/ops/reconciliation/evidence` expose read-only evidence with structured `503` behavior and no settlement, approval, or AI-promotion authority; ready-PostgreSQL route checks pass. |
