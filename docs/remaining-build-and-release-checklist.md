...
| Verifier cursor evidence | Strict durable cursor evidence contract | Implemented locally | Added `backend:release:verifier:cursor:evidence:check`; requires a ready verifier report, Base Sepolia chain ID `84532`, `verifierStatus.status=fresh`, a nonnegative `last_scanned_block`, parseable `updated_at`, and `unlinkedEvidenceCount=0`. It is now a first-class release-gate matrix check and a required member of the `fresh-verifier` post-attestation stage; it fingerprints the source report, blocks missing/stale/malformed cursors, and preserves read-only/non-authoritative flags. Operations-quality reports now also emit a machine-readable `clearanceCriteria` string for every expected blocker. |
