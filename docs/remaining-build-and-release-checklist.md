...
| CV | Release-gate operations-quality integration | Validated locally | `backend:operations:quality:check` includes `backend:release:gates:check` as an expected operator-blocked check; normal mode reports 10 checks, 7 expected blockers, and 0 unexpected failures. |
| CX | Latest durable release-gate operator endpoint | Validated locally | Authenticated read-only `GET /api/v2/ops/release-gates/latest` selects only `reportKind: release_gates` audit rows and returns structured `503` when no durable release-gate run exists. |
| CY | Release-gate and evidence workflow documentation | Validated locally | Updated the v2 operations and lineage contract plus batch documentation with durable release-gate provenance and non-releaseable operator evidence semantics. |
