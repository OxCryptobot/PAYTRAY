...
| CN | Evidence-bundle operations-quality integration | Validated locally | The nine-check `backend:operations:quality:check` matrix now includes the bundle CLI; normal mode classifies the missing real release evidence as `operator_blocked` with zero unexpected failures, while strict mode remains fail-closed. |
