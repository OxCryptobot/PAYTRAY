...
| DF | End-to-end Ed25519 operator-key custody and fingerprint verification | Validated locally | Added `backend:release:key:custody:check`, in-memory Ed25519 private/public-key derivation matching, exact public-key PEM SHA-256 comparison, approved-secret-manager source/version/protection checks, independent security fingerprint attestation binding, secret-free output, and `operator-key-custody` expected-blocked integration in the 20-check release-gate matrix. |
