...
| CG | Operations-quality matrix | Pushed | `d4e65f0`; `backend:operations:quality:check` runs eight existing read-only checks; normal mode exits `0` with `status: operator_blocked` and five expected operator blockers, while strict mode exits `1` until all target evidence is real. |
...
# CI also runs this command in a separate non-strict operations-quality job.
