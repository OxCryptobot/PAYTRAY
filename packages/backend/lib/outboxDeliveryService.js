...
function statusFor(row, maxAttempts) {
  ...
  if (row.dead_lettered_at || Number(row.attempts) >= maxAttempts) return 'dead'
  if (row.last_error) return 'failed'
  if ((row.lease_expires_at && new Date(row.lease_expires_at).getTime() > Date.now()) || (row.available_at && new Date(row.available_at).getTime() > Date.now())) return 'leased'
  return 'pending'
}
...
export async function markOutboxProcessed({ client, eventId, leaseToken = null }) {
  const result = await client.query(`
    ...
    WHERE id = $1
      ...
      AND ($2::uuid IS NULL OR lease_token = $2::uuid)
    RETURNING id, processed_at
  `, [required(eventId, 'eventId'), leaseToken])
  return result.rows[0] || null
}

export async function recordOutboxFailure({ client, eventId, leaseToken = null, error, retryBaseDelayMs = 1000, maxAttempts = 5 }) {
  ...
  const result = await client.query(`
    ...
    SET last_error = $2,
        ...
        dead_lettered_at = CASE WHEN attempts >= $3 THEN COALESCE(dead_lettered_at, CURRENT_TIMESTAMP) ELSE NULL END,
        lease_token = NULL,
        lease_acquired_at = NULL,
        lease_expires_at = NULL,
        available_at = CASE
    ...
    RETURNING *
  `, [required(eventId, 'eventId'), message, boundedMaxAttempts, boundedDelay, leaseToken])
...
export async function getOutboxHealth({ client, maxAttempts = 5, now = new Date() }) {
  ...
  const result = await client.query(`
    SELECT
      ...
      COUNT(*) FILTER (WHERE processed_at IS NULL AND lease_expires_at > CURRENT_TIMESTAMP AND dead_lettered_at IS NULL)::int AS leased,
      COUNT(*) FILTER (WHERE processed_at IS NULL AND last_error IS NOT NULL AND attempts < $1)::int AS failed,
      COUNT(*) FILTER (WHERE processed_at IS NULL AND (dead_lettered_at IS NOT NULL OR attempts >= $1))::int AS dead,
...
export async function listOutboxEvents({ client, limit = 50, offset = 0, status = null, maxAttempts = 5 }) {
  ...
  const result = await client.query(`
    ...
    WHERE ($3::text IS NULL
      ...
      OR ($3 = 'dead' AND processed_at IS NULL AND (dead_lettered_at IS NOT NULL OR attempts >= $4))
      OR ($3 = 'failed' AND processed_at IS NULL AND last_error IS NOT NULL AND attempts < $4 AND dead_lettered_at IS NULL)
      OR ($3 = 'leased' AND processed_at IS NULL AND dead_lettered_at IS NULL AND lease_expires_at > CURRENT_TIMESTAMP)
