...
export function createSablierFlowV3Decoder({ chainId = BASE_SEPOLIA_CHAIN_ID, contractAddress = BASE_SEPOLIA_SABLIER_FLOW_V3, getStreamContext = async () => null } = {}) {
  ...
  return async function decodeSablierFlowV3Log(log) {
    ...
    if (parsed.name === 'RefundFromFlowStream') return eventBase({ parsed, log, chainId: Number(chainId), contractAddress: normalizedContract, type: 'stream_refunded', streamId: args.streamId, senderWallet, recipientWallet, tokenAddress, amountBaseUnits: args.amount })
