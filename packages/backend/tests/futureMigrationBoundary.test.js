...
describe('migration-021/022 future boundary contract', () => {
  it('verifies the real repository remains explicitly not_present for 021 and 022', async () => {
    ...
    expect(workflow).toContain('Upload migration audit evidence')
    expect(workflow).toContain('name: migration-audits-${{ github.sha }}')
    expect(workflow).toContain('artifacts/migration-future-boundary.json.sha256')
